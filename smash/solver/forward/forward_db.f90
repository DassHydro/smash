...
CONTAINS
!  Differentiation of vic_infiltration in forward (tangent) mode (with options fixinterface):
...
!  Differentiation of vic_infiltration in reverse (adjoint) mode (with options fixinterface):
...
!  Differentiation of vic_vertical_transfer in forward (tangent) mode (with options fixinterface):
...
!  Differentiation of vic_vertical_transfer in reverse (adjoint) mode (with options fixinterface):
...
!  Differentiation of vic_interflow in forward (tangent) mode (with options fixinterface):
...
!  Differentiation of vic_interflow in reverse (adjoint) mode (with options fixinterface):
...
!  Differentiation of vic_baseflow in forward (tangent) mode (with options fixinterface):
...
!  Differentiation of vic_baseflow in reverse (adjoint) mode (with options fixinterface):
...
!  Differentiation of brooks_and_corey_flow in forward (tangent) mode (with options fixinterface):
...
!  Differentiation of brooks_and_corey_flow in reverse (adjoint) mode (with options fixinterface):
...
!  Differentiation of linear_evapotranspiration in forward (tangent) mode (with options fixinterface):
!   variations   of useful results: flow
...
!  Differentiation of linear_evapotranspiration in reverse (adjoint) mode (with options fixinterface):
...
!                this.cst:out this.exc:out this.lr:out
  SUBROUTINE PARAMETERSDT_INITIALISE_BWD(this, this_b, mesh)
    ...
    DEALLOCATE(this%ds)
    DEALLOCATE(this%ks)
    DEALLOCATE(this%clsl)
    DEALLOCATE(this%cusl2)
    DEALLOCATE(this%cusl1)
    DEALLOCATE(this%b)
    DEALLOCATE(this%exc)
...
!                this.hst:out this.hlr:out
  SUBROUTINE STATESDT_INITIALISE_BWD(this, this_b, mesh)
    ...
    DEALLOCATE(this%hlsl)
    DEALLOCATE(this%husl2)
    DEALLOCATE(this%husl1)
    DEALLOCATE(this%hst)
...
CONTAINS
!  Differentiation of cost_sign in forward (tangent) mode (with options fixinterface):
!   variations   of useful results: res
!   with respect to varying inputs: qs
  FUNCTION COST_SIGN_D(po, qo, qs, qs_d, mask_e, stype, res) RESULT (&
& RES_D)
    IMPLICIT NONE
    ...
    IF (stype(:1) .EQ. 'E') THEN
      DO i=1,SIZE(mask_e)
        IF (mask_e(i) .GT. n_event) n_event = mask_e(i)
      END DO
      res_d = 0.0
      DO i=1,n_event
        tmp_o = 0.
        ...
        CASE ('Epf') 
          tmp_s_d = 0.0
          DO j=1,SIZE(mask_e)
            IF (mask_e(j) .EQ. i .AND. qo(j) .GT. tmp_o) tmp_o = qo(j)
            IF (mask_e(j) .EQ. i .AND. qs(j) .GT. tmp_s) THEN
              tmp_s_d = qs_d(j)
              tmp_s = qs(j)
            END IF
          END DO
        CASE ('Elt') 
          ...
          tmp = 0.
          DO j=1,SIZE(mask_e)
            IF (mask_e(j) .EQ. i .AND. qo(j) .GT. tmp_o) THEN
              tmp_o = qo(j)
              indm_qo = j
            END IF
            IF (mask_e(j) .EQ. i .AND. qs(j) .GT. tmp_s) THEN
              ...
              indm_qs = j
            END IF
            IF (mask_e(j) .EQ. i .AND. po(j) .GT. tmp) THEN
              tmp = po(j)
              indm_po = j
            END IF
          END DO
        ...
        CASE ('Erc') 
          ...
          tmp_s_d = 0.0
          DO j=1,SIZE(mask_e)
            IF (mask_e(j) .EQ. i .AND. po(j) .GE. 0 .AND. qo(j) .GE. 0 &
&               .AND. qs(j) .GE. 0) THEN
              tmp = tmp + po(j)
              tmp_o = tmp_o + qo(j)
              tmp_s_d = tmp_s_d + qs_d(j)
              tmp_s = tmp_s + qs(j)
            END IF
          END DO
          IF (tmp .GT. 0) tmp_o = tmp_o/tmp
          IF (tmp .GT. 0) THEN
            tmp_s_d = tmp_s_d/tmp
            tmp_s = tmp_s/tmp
          END IF
        ...
        END SELECT
        IF (tmp_o .GT. 0) THEN
          IF (tmp_s/tmp_o - 1 .GE. 0.) THEN
            abs0_d = tmp_s_d/tmp_o
          ELSE
            abs0_d = -(tmp_s_d/tmp_o)
          END IF
          res_d = res_d + abs0_d
    ...
    ELSE
      res_d = 0.0
    END IF
  END FUNCTION COST_SIGN_D

!  Differentiation of cost_sign in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: res qs
!   with respect to varying inputs: qs
  SUBROUTINE COST_SIGN_B(po, qo, qs, qs_b, mask_e, stype, res_b)
    IMPLICIT NONE
    REAL, DIMENSION(:), INTENT(IN) :: po, qo, qs
    REAL, DIMENSION(:) :: qs_b
    INTEGER, DIMENSION(:), INTENT(IN) :: mask_e
...
&               .AND. qs(j) .GE. 0) THEN
      ...
      DO i=n_event,1,-1
        ...
        ELSE IF (branch .EQ. 2) THEN
          ...
          CALL POPREAL4(tmp)
        ELSE
          IF (branch .EQ. 3) tmp_s_b = tmp_s_b/tmp
          CALL POPINTEGER4(ad_to2)
          DO j=ad_to2,1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) qs_b(j) = qs_b(j) + tmp_s_b
          END DO
          CALL POPREAL4(tmp)
        END IF
        CALL POPREAL4(tmp_o)
    ...
    ELSE IF (stype(:1) .EQ. 'C') THEN
      ...
      CALL POPCONTROL2B(branch)
      IF (branch .NE. 0) THEN
        IF (branch .EQ. 1) tmp_s_b = tmp_s_b/tmp
        CALL POPINTEGER4(ad_to3)
        DO i=ad_to3,1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) qs_b(i) = qs_b(i) + tmp_s_b
        END DO
      END IF
    END IF
  END SUBROUTINE COST_SIGN_B
    ...
    IF (stype(:1) .EQ. 'E') THEN
      ...
      DO i=1,n_event
        ...
        CASE ('Epf') 
          DO j=1,SIZE(mask_e)
            IF (mask_e(j) .EQ. i .AND. qo(j) .GT. tmp_o) tmp_o = qo(j)
            IF (mask_e(j) .EQ. i .AND. qs(j) .GT. tmp_s) tmp_s = qs(j)
          END DO
        CASE ('Elt') 
          indm_po = 0
          indm_qo = 0
...
&       mesh%dx)*1e3_sp
      DO j=1,setup%optimize%njf
        IF (ANY(qo .GE. 0._sp)) THEN
          ...
          SELECT CASE  (jn_tmp) 
          CASE ('nse') 
            j_tmp_d = NSE_D(qo, qs, qs_d, j_tmp)
          CASE ('kge') 
            j_tmp_d = KGE_D(qo, qs, qs_d, j_tmp)
          CASE ('kge2') 
            imd_d = KGE_D(qo, qs, qs_d, imd)
            j_tmp_d = 2*imd*imd_d
          CASE ('se') 
            j_tmp_d = SE_D(qo, qs, qs_d, j_tmp)
          CASE ('rmse') 
            j_tmp_d = RMSE_D(qo, qs, qs_d, j_tmp)
          CASE ('logarithmic') 
            j_tmp_d = LOGARITHMIC_D(qo, qs, qs_d, j_tmp)
          CASE ('Crc', 'Erc', 'Elt', 'Epf') 
! CASE OF SIGNATURES
            j_tmp_d = COST_SIGN_D(po, qo, qs, qs_d, setup%optimize%&
&             mask_event(g, setup%optimize%optimize_start_step:setup%&
&             ntime_step), jn_tmp, j_tmp)
          END SELECT
        END IF
...
&       mesh%dx)*1e3_sp
      DO j=1,setup%optimize%njf
        IF (ANY(qo .GE. 0._sp)) THEN
          ...
          CASE ('Crc', 'Erc', 'Elt', 'Epf') 
! CASE OF SIGNATURES
            res4 = COST_SIGN(po, qo, qs, setup%optimize%mask_event(g, &
...
&       jobs_b
    ...
    DO g=mesh%ng,1,-1
      ...
      DO j=setup%optimize%njf,1,-1
        j_tmp_b = j_tmp_b + setup%optimize%wjobs_fun(j)*gauge_jobs_b(g)
        ...
        ELSE IF (branch .EQ. 7) THEN
          jn_tmp = setup%optimize%jobs_fun(j)
          res_b4 = j_tmp_b
          CALL COST_SIGN_B(po, qo, qs, qs_b, setup%optimize%mask_event(g&
&                    , setup%optimize%optimize_start_step:setup%&
&                    ntime_step), jn_tmp, res_b4)
          j_tmp_b = 0.0
        END IF
      END DO
...
&       mesh%dx)*1e3_sp
      DO j=1,setup%optimize%njf
        IF (ANY(qo .GE. 0._sp)) THEN
          ...
          CASE ('Crc', 'Erc', 'Elt', 'Epf') 
! CASE OF SIGNATURES
            j_tmp = COST_SIGN(po, qo, qs, setup%optimize%mask_event(g, &
&             setup%optimize%optimize_start_step:setup%ntime_step), &
&             jn_tmp)
          END SELECT
        END IF
...
!                *(hyper_states.hst):in hyper_states.husl1:(loc)
!                *(hyper_states.husl1):in hyper_states.husl2:(loc)
!                *(hyper_states.husl2):in hyper_states.hlsl:(loc)
!                *(hyper_states.hlsl):in hyper_states.hlr:(loc)
...
!                *(hyper_parameters.exc):in hyper_parameters.b:(loc)
!                *(hyper_parameters.b):in hyper_parameters.cusl1:(loc)
!                *(hyper_parameters.cusl1):in hyper_parameters.cusl2:(loc)
!                *(hyper_parameters.cusl2):in hyper_parameters.clsl:(loc)
!                *(hyper_parameters.clsl):in hyper_parameters.ks:(loc)
!                *(hyper_parameters.ks):in hyper_parameters.ds:(loc)
!                *(hyper_parameters.ds):in hyper_parameters.dsm:(loc)
!                *(hyper_parameters.dsm):in hyper_parameters.ws:(loc)
!                *(hyper_parameters.ws):in hyper_parameters.lr:(loc)
...
!  Differentiation of vic_base_forward in forward (tangent) mode (with options fixinterface):
!   variations   of useful results: cost
!   with respect to varying inputs: *(parameters.ci) *(parameters.cp)
!                *(parameters.beta) *(parameters.cft) *(parameters.cst)
!                *(parameters.alpha) *(parameters.exc) *(parameters.b)
...
!                states.hst:in-out states.husl1:in-out states.husl2:in-out
!                states.hlsl:in-out states.hlr:in-out
SUBROUTINE VIC_BASE_FORWARD_D(setup, mesh, input_data, parameters, &
& parameters_d, parameters_bgd, states, states_d, states_bgd, output, &
& output_d, cost, cost_d)
!% only: sp
  USE MD_CONSTANT
...
&               , parameters_bgd, states, states_d, states_bgd, output, &
&               output_d, cost, cost_d)
END SUBROUTINE VIC_BASE_FORWARD_D

!  Differentiation of vic_base_forward in reverse (adjoint) mode (with options fixinterface):
!   gradient     of useful results: cost
!   with respect to varying inputs: *(parameters.ci) *(parameters.cp)
!                *(parameters.beta) *(parameters.cft) *(parameters.cst)
!                *(parameters.alpha) *(parameters.exc) *(parameters.b)
...
!                states.hst:in-out states.husl1:in-out states.husl2:in-out
!                states.hlsl:in-out states.hlr:in-out
SUBROUTINE VIC_BASE_FORWARD_B(setup, mesh, input_data, parameters, &
& parameters_b, parameters_bgd, states, states_b, states_bgd, output, &
& output_b, cost, cost_b)
!% only: sp
  USE MD_CONSTANT
...
&               output_b, cost, cost_b)
  DO t=setup%ntime_step,1,-1
    DO g=mesh%ng,1,-1
      ...
      CALL POPINTEGER4(row)
    END DO
    DO 110 i=mesh%nrow*mesh%ncol,1,-1
      CALL POPCONTROL3B(branch)
      IF (branch .LT. 2) THEN
...
&                           cusl2(row, col), parameters_b%cusl2(row, col&
&                           ), parameters%b(row, col), parameters_b%b(&
&                           row, col), states%husl1(row, col), states_b%&
&                           husl1(row, col), states%husl2(row, col), &
&                           states_b%husl2(row, col), runoff, runoff_b)
        END IF
 ...
 110 CONTINUE
  ...
  ELSE
    ...
    DEALLOCATE(sparse_q_b)
  END IF
END SUBROUTINE VIC_BASE_FORWARD_B

SUBROUTINE VIC_BASE_FORWARD_NODIFF(setup, mesh, input_data, parameters, &
& parameters_bgd, states, states_bgd, output, cost)
!% only: sp
  USE MD_CONSTANT
!% only: SetupDT
...
!% =================================================================================================================== %!
  CALL COMPUTE_COST(setup, mesh, input_data, parameters, parameters_bgd&
&             , states, states_bgd, output, cost)
END SUBROUTINE VIC_BASE_FORWARD_NODIFF

!  Differentiation of vic_base_hyper_forward in forward (tangent) mode (with options fixinterface):
!   RW status of diff variables: hyper_states.hi:(loc) *(hyper_states.hi):(loc)
!                hyper_states.hp:(loc) *(hyper_states.hp):(loc)
!                hyper_states.hft:(loc) *(hyper_states.hft):(loc)
!                hyper_states.hst:(loc) *(hyper_states.hst):(loc)
!                hyper_states.husl1:(loc) *(hyper_states.husl1):(loc)
!                hyper_states.husl2:(loc) *(hyper_states.husl2):(loc)
!                hyper_states.hlsl:(loc) *(hyper_states.hlsl):(loc)
!                hyper_states.hlr:(loc) *(hyper_states.hlr):(loc)
!                hyper_parameters.ci:(loc) *(hyper_parameters.ci):(loc)
!                hyper_parameters.cp:(loc) *(hyper_parameters.cp):(loc)
!                hyper_parameters.beta:(loc) *(hyper_parameters.beta):(loc)
!                hyper_parameters.cft:(loc) *(hyper_parameters.cft):(loc)
!                hyper_parameters.cst:(loc) *(hyper_parameters.cst):(loc)
!                hyper_parameters.alpha:(loc) *(hyper_parameters.alpha):(loc)
!                hyper_parameters.exc:(loc) *(hyper_parameters.exc):(loc)
!                hyper_parameters.b:(loc) *(hyper_parameters.b):(loc)
!                hyper_parameters.cusl1:(loc) *(hyper_parameters.cusl1):(loc)
!                hyper_parameters.cusl2:(loc) *(hyper_parameters.cusl2):(loc)
!                hyper_parameters.clsl:(loc) *(hyper_parameters.clsl):(loc)
!                hyper_parameters.ks:(loc) *(hyper_parameters.ks):(loc)
!                hyper_parameters.ds:(loc) *(hyper_parameters.ds):(loc)
!                hyper_parameters.dsm:(loc) *(hyper_parameters.dsm):(loc)
...
!% =================================================================================================================== %!
  ...
  ELSE
    ...
    cost_d = 0.0_4
  END IF
END SUBROUTINE VIC_BASE_HYPER_FORWARD_D

!  Differentiation of vic_base_hyper_forward in reverse (adjoint) mode (with options fixinterface):
!   RW status of diff variables: hyper_states.hi:(loc) *(hyper_states.hi):(loc)
!                hyper_states.hp:(loc) *(hyper_states.hp):(loc)
!                hyper_states.hft:(loc) *(hyper_states.hft):(loc)
!                hyper_states.hst:(loc) *(hyper_states.hst):(loc)
...
!% =================================================================================================================== %!
  ...
  ELSE
    ALLOCATE(q(mesh%nrow, mesh%ncol))
  END IF
END SUBROUTINE VIC_BASE_HYPER_FORWARD_NODIFF
