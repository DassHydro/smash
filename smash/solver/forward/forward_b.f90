...
END MODULE M_OPERATOR_DIFF_B

!%    This module `mw_states` encapsulates all SMASH states
MODULE MW_STATES_DIFF_B
!% only: sp, dp, lchar, np, ns
  USE M_COMMON
!% only: SetupDT
  USE MW_SETUP
...
END MODULE MW_PARAMETERS_DIFF_B
...
!% only: ParametersDT, parameters_derived_type_to_matrix
  USE MW_PARAMETERS_DIFF_B
!% only: StatesDT
  USE MW_STATES_DIFF_B
!% only: OutputDT
  USE MW_OUTPUT_DIFF_B
!% only: GR_interception, GR_production, GR_exchange, &
  USE M_OPERATOR_DIFF_B
...
& qr_b, qt_b, qup_b
  ...
  INTEGER :: branch
  IF (setup%sparse_storage) THEN
    ALLOCATE(sparse_q_b(mesh%nac, setup%ntime_step))
    sparse_q_b = 0.0_4
    ALLOCATE(sparse_q(mesh%nac, setup%ntime_step))
    CALL PUSHCONTROL1B(0)
  ELSE
    ALLOCATE(q_b(mesh%nrow, mesh%ncol, setup%ntime_step))
    q_b = 0.0_4
    ALLOCATE(q(mesh%nrow, mesh%ncol, setup%ntime_step))
    CALL PUSHCONTROL1B(1)
  END IF
...
&               cost_b)
  DO g=mesh%ng,1,-1
    ...
    IF (branch .EQ. 0) THEN
      row_g = mesh%gauge_pos(1, g)
      col_g = mesh%gauge_pos(2, g)
      q_b(row_g, col_g, :) = q_b(row_g, col_g, :) + output_b%qsim(g, :)
      output_b%qsim(g, :) = 0.0_4
    ELSE
      row_g = mesh%gauge_pos(1, g)
      col_g = mesh%gauge_pos(2, g)
      k_g = mesh%rowcol_to_ind_sparse(row_g, col_g)
      sparse_q_b(k_g, :) = sparse_q_b(k_g, :) + output_b%qsim(g, :)
      output_b%qsim(g, :) = 0.0_4
    END IF
  END DO
    ...
    DO i=mesh%nrow*mesh%ncol,1,-1
      ...
      ELSE
        IF (branch .EQ. 2) THEN
          row = mesh%path(1, i)
          col = mesh%path(2, i)
          temp_b0 = setup%dx**2*0.001_sp*q_b(row, col, t)/setup%dt
          q_b(row, col, t) = 0.0_4
          qt_b = temp_b0
...
&                             drained_area, row, col, t, q, q_b, qup, &
&                             qup_b)
        ELSE IF (branch .EQ. 3) THEN
          row = mesh%path(1, i)
          col = mesh%path(2, i)
          temp_b0 = setup%dx**2*0.001_sp*sparse_q_b(k, t)/setup%dt
          sparse_q_b(k, t) = 0.0_4
          qt_b = temp_b0
          qup_b = REAL(mesh%drained_area(row, col)-1)*temp_b0
...
&                                    sparse_q, sparse_q_b, qup, qup_b)
        ...
        ELSE
          ...
          CALL GR_TRANSFERN_B(5._sp, prcp, prr, prr_b, parameters%cft(&
...
&                        col), parameters_b%cp(row, col), parameters%&
&                        beta(row, col), parameters_b%beta(row, col), &
