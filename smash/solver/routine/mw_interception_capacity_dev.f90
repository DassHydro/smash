...
module mw_interception_capacity_dev
    ...
    use md_gr_operator_dev, only: gr_interception_dev
    use m_array_creation_dev, only: arange_dev

    implicit none

contains

    subroutine compute_interception_capacity_dev(setup, mesh, input_data, day_index, nday, ci)

        implicit none

        type(SetupDT_dev), intent(in) :: setup
        type(MeshDT_dev), intent(in) :: mesh
        type(Input_DataDT_dev), intent(in) :: input_data
        ...
        call arange_dev(stt, stp, step, cmax)
            ...
            do j = 1, setup%ntime_step
                ...
                do col = 1, mesh%ncol
                    ...
                    do row = 1, mesh%nrow
                        ...
                        call gr_interception_dev(prcp, pet, cmax(i), h(row, col), pth, ec)
    ...
    end subroutine compute_interception_capacity_dev

end module mw_interception_capacity_dev
