...
!  Differentiation of hortonian_production in forward (tangent) mode (with options fixinterface noISIZE OpenMP context):
!   variations   of useful results: hp perc pr
!   with respect to varying inputs: hp en cp pn
  SUBROUTINE HORTONIAN_PRODUCTION_D(pn, pn_d, en, en_d, cp, cp_d, beta, &
&   hp, hp_d, pr, pr_d, perc, perc_d, dt)
    IMPLICIT NONE
    REAL(sp), INTENT(IN) :: pn, en, cp, beta
    REAL(sp), INTENT(IN) :: pn_d, en_d, cp_d
...
!  Differentiation of hortonian_production in reverse (adjoint) mode (with options fixinterface noISIZE OpenMP context):
!   gradient     of useful results: hp cp pn perc pr
!   with respect to varying inputs: hp en cp pn
  SUBROUTINE HORTONIAN_PRODUCTION_B(pn, pn_b, en, en_b, cp, cp_b, beta, &
&   hp, hp_b, pr, pr_b, perc, perc_b, dt)
    IMPLICIT NONE
    REAL(sp), INTENT(IN) :: pn, en, cp, beta
    REAL(sp) :: pn_b, en_b, cp_b
    REAL(sp), INTENT(IN) :: dt
...
!  Differentiation of gr5_hortonian_time_step in forward (tangent) mode (with options fixinterface noISIZE OpenMP context):
!   variations   of useful results: ac_qt ac_hi ac_hp ac_ht
!   with respect to varying inputs: ac_kexc ac_ci ac_cp ac_ct ac_qt
!                ac_hi ac_hp ac_ht ac_mlt ac_aexc
...
&                                 ac_cp_d(k), beta, ac_hp(k), ac_hp_d(k)&
&                                 , pr, pr_d, perc, perc_d, setup%dt)
            CALL GR_THRESHOLD_EXCHANGE_D(ac_kexc(k), ac_kexc_d(k), &
&                                  ac_aexc(k), ac_aexc_d(k), ac_ht(k), &
&                                  ac_ht_d(k), l, l_d)
          ELSE
...
!  Differentiation of gr5_hortonian_time_step in reverse (adjoint) mode (with options fixinterface noISIZE OpenMP context):
!   gradient     of useful results: ac_kexc ac_ci ac_cp ac_ct ac_qt
!                ac_hi ac_hp ac_ht ac_mlt ac_aexc
!   with respect to varying inputs: ac_kexc ac_ci ac_cp ac_ct ac_qt
!                ac_hi ac_hp ac_ht ac_mlt ac_aexc
...
&                          k), pn, en)
            ...
            CALL PUSHREAL4(pr)
            CALL PUSHREAL4(ac_hp(k))
            CALL HORTONIAN_PRODUCTION(pn, en, ac_cp(k), beta, ac_hp(k), &
&                               pr, perc, setup%dt)
            CALL GR_THRESHOLD_EXCHANGE(ac_kexc(k), ac_aexc(k), ac_ht(k)&
&                                , l)
            CALL PUSHCONTROL1B(1)
...
&                                 ac_cp_b(k), beta, ac_hp(k), ac_hp_b(k)&
&                                 , pr, pr_b, perc, perc_b, setup%dt)
            CALL POPREAL4(ac_hi(k))
            CALL POPREAL4(pn)
            CALL POPREAL4(en)
            CALL GR_INTERCEPTION_B(ac_prcp(k), ac_prcp_b(k), ac_pet(k), &
