...
!  Differentiation of apply_discharge_tfm in forward (tangent) mode (with options fixinterface noISIZE OpenMP context):
!   variations   of useful results: qs
!   with respect to varying inputs: qs
  SUBROUTINE APPLY_DISCHARGE_TFM_D(tfm_name, qo, qo_d, qs, qs_d)
    IMPLICIT NONE
    CHARACTER(len=lchar), INTENT(IN) :: tfm_name
    REAL(sp), DIMENSION(:), INTENT(INOUT) :: qo, qs
    REAL(sp), DIMENSION(:), INTENT(INOUT) :: qo_d, qs_d
    INTEGER :: i
    REAL(sp) :: mean_qo, e
    INTRINSIC SIZE
...
!% Should be reach by "keep" only. Do nothing
    ...
    CASE ('inv') 
      ...
      END WHERE
    END SELECT
  END SUBROUTINE APPLY_DISCHARGE_TFM_D

!  Differentiation of apply_discharge_tfm in reverse (adjoint) mode (with options fixinterface noISIZE OpenMP context):
!   gradient     of useful results: qs
!   with respect to varying inputs: qs
  SUBROUTINE APPLY_DISCHARGE_TFM_B(tfm_name, qo, qo_b, qs, qs_b)
    IMPLICIT NONE
    CHARACTER(len=lchar), INTENT(IN) :: tfm_name
    REAL(sp), DIMENSION(:), INTENT(INOUT) :: qo, qs
    REAL(sp), DIMENSION(:), INTENT(INOUT) :: qo_b, qs_b
    INTEGER :: i
    REAL(sp) :: mean_qo, e
    INTRINSIC SIZE
    LOGICAL, DIMENSION(SIZE(qo)) :: mask
...
!% Should be reach by "keep" only. Do nothing
    ...
    END SELECT
  END SUBROUTINE APPLY_DISCHARGE_TFM_B
...
&         setup%ntime_step)
        DO j=1,options%cost%njoc
          qo = input_data%response_data%q(i, options%cost%end_warmup:&
&           setup%ntime_step)
...
&           ntime_step)
          WHERE (qo .LT. 0._sp) 
            ...
            qs = -99._sp
          END WHERE
          CALL APPLY_DISCHARGE_TFM_D(options%cost%jobs_cmpt_tfm(j), qo, &
&                              qo_d, qs, qs_d)
...
&                      end_warmup + 1)
          CALL APPLY_DISCHARGE_TFM_B(options%cost%jobs_cmpt_tfm(j), qo, &
&                              qo_b, qs, qs_b)
          CALL POPREAL4ARRAY(qs, setup%ntime_step - options%cost%&
&                      end_warmup + 1)
...
&         ntime_step) = output_b%response%q(i, options%cost%end_warmup:&
