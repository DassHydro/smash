...
! Beta percolation parameter is time step dependent
    beta = 9._sp/4._sp*(86400._sp/setup%dt)**0.25_sp
!$OMP PARALLEL DO NUM_THREADS(options%comm%ncpu), SHARED(setup, mesh, &
!$OMP&ac_prcp, ac_pet, ac_ci, ac_cp, beta, ac_ct, ac_be, ac_kexc, &
!$OMP&ac_aexc, ac_hi, ac_hp, ac_ht, ac_he, ac_qt), SHARED(ac_prcp_d, &
!$OMP&ac_ci_d, ac_cp_d, ac_ct_d, ac_be_d, ac_kexc_d, ac_aexc_d, ac_hi_d&
!$OMP&, ac_hp_d, ac_ht_d, ac_he_d, ac_qt_d), PRIVATE(row, col, k, pn, en&
!$OMP&, pr, perc, l, prr, pre, prd, qr, qd, qe), PRIVATE(pn_d, en_d, &
!$OMP&pr_d, perc_d, l_d, prr_d, pre_d, prd_d, qr_d, qd_d, qe_d), PRIVATE&
...
!$OMP&pn, en, pr, perc, l, prr, pre, prd, qr, qd, qe), PRIVATE(&
...
!$OMP&ac_ci_b, ac_cp_b, ac_ct_b, ac_be_b, ac_kexc_b, ac_aexc_b, ac_hi_b&
!$OMP&, ac_hp_b, ac_ht_b, ac_he_b, ac_qt_b), PRIVATE(row, col, k, pn, en&
!$OMP&, pr, perc, l, prr, pre, prd, qr, qd, qe), PRIVATE(pn_b, en_b, &
!$OMP&pr_b, perc_b, l_b, prr_b, pre_b, prd_b, qr_b, qd_b, qe_b), PRIVATE&
...
!$OMP&ac_prcp, ac_pet, ac_ci, ac_cp, beta, ac_ct, ac_be, ac_kexc, &
!$OMP&ac_aexc, ac_hi, ac_hp, ac_ht, ac_he, ac_qt), PRIVATE(row, col, k, &
!$OMP&pn, en, pr, perc, l, prr, pre, prd, qr, qd, qe), SCHEDULE(static)
    DO col=1,mesh%ncol
      DO row=1,mesh%nrow
        IF (.NOT.(mesh%active_cell(row, col) .EQ. 0 .OR. mesh%&
&           local_active_cell(row, col) .EQ. 0)) THEN
...
!                ac_qt ac_hi ac_hl ac_hp ac_ht ac_mlt
  SUBROUTINE GRC_TIME_STEP_D(setup, mesh, input_data, options, time_step&
&   , ac_mlt, ac_mlt_d, ac_ci, ac_ci_d, ac_cp, ac_cp_d, ac_ct, ac_ct_d, &
&   ac_cl, ac_cl_d, ac_kexc, ac_kexc_d, ac_hi, ac_hi_d, ac_hp, ac_hp_d, &
&   ac_ht, ac_ht_d, ac_hl, ac_hl_d, ac_qt, ac_qt_d)
    IMPLICIT NONE
    TYPE(SETUPDT), INTENT(IN) :: setup
    TYPE(MESHDT), INTENT(IN) :: mesh
    TYPE(INPUT_DATADT), INTENT(IN) :: input_data
...
&                              , 'pet', ac_pet)
    ...
    ac_prcp = ac_prcp + ac_mlt
!$OMP PARALLEL DO NUM_THREADS(options%comm%ncpu), SHARED(setup, mesh, &
!$OMP&ac_prcp, ac_pet, ac_ci, ac_cp, ac_ct, ac_cl, ac_kexc, ac_hi, ac_hp&
!$OMP&, ac_ht, ac_hl, ac_qt), SHARED(ac_prcp_d, ac_ci_d, ac_cp_d, &
!$OMP&ac_ct_d, ac_cl_d, ac_kexc_d, ac_hi_d, ac_hp_d, ac_ht_d, ac_hl_d, &
!$OMP&ac_qt_d), PRIVATE(row, col, k, pn, en, pr, perc, l, prr, prl, prd&
!$OMP&, qr, ql, qd), PRIVATE(pn_d, en_d, pr_d, perc_d, l_d, prr_d, prl_d&
!$OMP&, prd_d, qr_d, ql_d, qd_d), PRIVATE(temp), SCHEDULE(static)
    DO col=1,mesh%ncol
      DO row=1,mesh%nrow
        IF (.NOT.(mesh%active_cell(row, col) .EQ. 0 .OR. mesh%&
&           local_active_cell(row, col) .EQ. 0)) THEN
...
&                        ac_ht_d(k), l, l_d)
          ELSE
            ...
            pr_d = 0.0_4
          END IF
          prr_d = 0.54_sp*(pr_d+perc_d) + l_d
          prr = 0.54_sp*(pr+perc) + l
          prl_d = 0.36_sp*(pr_d+perc_d)
          prl = 0.36_sp*(pr+perc)
          prd_d = 0.1_sp*(pr_d+perc_d)
          prd = 0.1_sp*(pr+perc)
          CALL GR_TRANSFER_D(5._sp, ac_prcp(k), prr, prr_d, ac_ct(k), &
...
!   with respect to varying inputs: ac_kexc ac_ci ac_cl ac_cp ac_ct
!                ac_qt ac_hi ac_hl ac_hp ac_ht ac_mlt
  SUBROUTINE GRC_TIME_STEP_B(setup, mesh, input_data, options, time_step&
&   , ac_mlt, ac_mlt_b, ac_ci, ac_ci_b, ac_cp, ac_cp_b, ac_ct, ac_ct_b, &
&   ac_cl, ac_cl_b, ac_kexc, ac_kexc_b, ac_hi, ac_hi_b, ac_hp, ac_hp_b, &
&   ac_ht, ac_ht_b, ac_hl, ac_hl_b, ac_qt, ac_qt_b)
    IMPLICIT NONE
    TYPE(SETUPDT), INTENT(IN) :: setup
    TYPE(MESHDT), INTENT(IN) :: mesh
    TYPE(INPUT_DATADT), INTENT(IN) :: input_data
...
!$OMP PARALLEL NUM_THREADS(options%comm%ncpu), SHARED(setup, mesh, &
!$OMP&ac_prcp, ac_pet, ac_ci, ac_cp, ac_ct, ac_cl, ac_kexc, ac_hi, ac_hp&
!$OMP&, ac_ht, ac_hl, ac_qt), PRIVATE(row, col, k, pn, en, pr, perc, l, &
...
!$OMP PARALLEL NUM_THREADS(options%comm%ncpu), SHARED(setup, mesh, &
!$OMP&ac_prcp, ac_pet, ac_ci, ac_cp, ac_ct, ac_cl, ac_kexc, ac_hi, ac_hp&
!$OMP&, ac_ht, ac_hl, ac_qt), SHARED(ac_prcp_b, ac_ci_b, ac_cp_b, &
!$OMP&ac_ct_b, ac_cl_b, ac_kexc_b, ac_hi_b, ac_hp_b, ac_ht_b, ac_hl_b, &
!$OMP&ac_qt_b), PRIVATE(row, col, k, pn, en, pr, perc, l, prr, prl, prd&
!$OMP&, qr, ql, qd), PRIVATE(pn_b, en_b, pr_b, perc_b, l_b, prr_b, prl_b&
!$OMP&, prd_b, qr_b, ql_b, qd_b), PRIVATE(branch, chunk_end, chunk_start&
!$OMP&)
    CALL POPREAL4(en)
    CALL POPREAL4(prl)
    CALL POPREAL4(prr)
...
&                      ac_cl_b(k), ac_hl(k), ac_hl_b(k), ql, ql_b)
          ...
          CALL GR_TRANSFER_B(5._sp, ac_prcp(k), prr, prr_b, ac_ct(k), &
&                      ac_ct_b(k), ac_ht(k), ac_ht_b(k), qr, qr_b)
          pr_b = 0.1_sp*prd_b + 0.36_sp*prl_b + 0.54_sp*prr_b
          perc_b = 0.1_sp*prd_b + 0.36_sp*prl_b + 0.54_sp*prr_b
          CALL POPREAL4(prl)
          CALL POPREAL4(prr)
          l_b = l_b + prr_b
          CALL POPCONTROL1B(branch)
...
!$OMP END PARALLEL
  ...
  SUBROUTINE GRC_TIME_STEP(setup, mesh, input_data, options, time_step, &
...
!% only: gr4_time_step, gr5_time_step, gr6_time_step, grc_time_step, grd_time_step, loieau_time_step
...
! % ht
! % hl
        CALL GRC_TIME_STEP_D(setup, mesh, input_data, options, t, &
...
&                     SIZE(checkpoint_variable%ac_qtz, 1))
        ...
        CALL PUSHREAL4ARRAY(h2, mesh%nac)
        CALL PUSHREAL4ARRAY(h1, mesh%nac)
        CALL GRC_TIME_STEP(setup, mesh, input_data, options, t, &
&                    checkpoint_variable%ac_mlt, checkpoint_variable%&
&                    ac_rr_parameters(:, rr_parameters_inc+1), &
&                    checkpoint_variable%ac_rr_parameters(:, &
&                    rr_parameters_inc+2), checkpoint_variable%&
...
&           h1_b
      ...
      ELSE IF (branch .LT. 6) THEN
        IF (branch .EQ. 4) THEN
          ...
          CALL POPREAL4ARRAY(checkpoint_variable%ac_qtz(:, setup%nqz), &
&                      SIZE(checkpoint_variable%ac_qtz, 1))
          CALL GRC_TIME_STEP_B(setup, mesh, input_data, options, t, &
