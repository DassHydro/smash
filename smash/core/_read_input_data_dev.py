...
from smash.core._constant_dev import RATIO_PET_HOURLY

from smash.solver._mw_sparse_storage_dev import matrix_to_sparse_vector_r_dev
...
def _read_windowed_raster_dev(path: str, mesh: MeshDT) -> np.ndarray:
    ds = gdal.Open(path)
...
def _read_qobs_dev(setup: SetupDT, mesh: MeshDT, input_data: Input_DataDT):
...
def _index_containing_substring_dev(the_list: list, substring: str):
...
def _adjust_left_files_dev(files: list[str], date_range: pd.Timestamp):
    n = 0
    ind = -1
    while ind == -1:
        ind = _index_containing_substring_dev(
...
def _read_prcp_dev(setup: SetupDT_dev, mesh: MeshDT_dev, input_data: Input_DataDT_dev):
    ...
    if setup.prcp_format == "tif":
        ...
        for i, date in enumerate(tqdm(date_range, desc="</> Reading precipitation")):
            ...
            if ind == -1:
                ...
                matrix = (
                    _read_windowed_raster_dev(files[ind], mesh)
...
def _read_pet_dev(setup: SetupDT, mesh: MeshDT, input_data: Input_DataDT):
    date_range = pd.date_range(
        start=setup.start_time,
        end=setup.end_time,
        freq=f"{int(setup.dt)}s",
        ...
        if setup.daily_interannual_pet:
            ...
            for i, day in enumerate(
                ...
                if day.day_of_year in date_range.day_of_year:
                    ...
                    ind = _index_containing_substring_dev(files, day_strf)
                    ...
                    if ind == -1:
                        ...
                        if setup.sparse_storage:
                            ...
                            matrix_to_sparse_vector_r_dev(mesh, matrix, vector)
...
def _read_descriptor_dev(setup: SetupDT, mesh: MeshDT, input_data: Input_DataDT):
    for i, name in enumerate(setup.descriptor_name):
        ...
        if len(path) == 0:
            ...
            input_data.physio_data.descriptor[..., i] = _read_windowed_raster_dev(
                path[0], mesh
            )
