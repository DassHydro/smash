...
from smash._constant import CSIGN, DOMAIN, ESIGN, EVENT_SEG_SIGNATURES_KEYS, SIGNS
...
from smash.core.signal_analysis.segmentation._standardize import (  # noqa: F401
    ...
    _standardize_hydrograph_segmentation_gauge,
...
def _standardize_signatures_gauge(
    model: Model,
    domain: str,
    gauge: str | ListLike,
) -> np.ndarray:
    return _standardize_hydrograph_segmentation_gauge(model, domain, gauge)
    ...
    if event_seg is None:
        ...
        if isinstance(event_seg, dict):
            for key, value in event_seg.items():
                if key in EVENT_SEG_SIGNATURES_KEYS:
                    func = eval(f"_standardize_hydrograph_segmentation_{key}")
                    event_seg[key] = func(value)
                else:
                    raise ValueError(
                        f"Unknown key '{key}' in event_seg. Choices: {EVENT_SEG_SIGNATURES_KEYS}"
                    )
        else:
...
def _standardize_signatures_args(
    ...
    gauge = _standardize_signatures_gauge(model, domain, gauge)

    event_seg = _standardize_signatures_event_seg(event_seg)

    return (cs, es, gauge, domain, event_seg)
