...
from smash._constant import OPR_PARAMETERS, OPR_STATES, LOW_FEASIBLE_OPR_PARAMETERS, LOW_OPR_INITIAL_STATES, UPP_OPR_INITIAL_STATES, LOW_OPTIM_OPR_PARAMETERS, UPP_OPTIM_OPR_PARAMETERS
...
def _standardize_paramstates(paramstate: ParametersDT, tol=1e-9):
    ...
    for i, param_name in enumerate(OPR_PARAMETERS):
        ...
        if np.any(param_array <= low_feas):
            raise ValueError(f"Invalid value for Model parameter {param_name} (<= {low_feas})")
        
        else:
            if np.logical_or(param_array < (low - tol), param_array > (upp + tol)).any():
                warnings.warn(f"Model parameter {param_name} is out of default boundary condition [{low}, {upp}]")
    ...
    for i, state_name in enumerate(OPR_STATES):
        ...
        if np.logical_or(state_array < (low - tol), state_array > (upp + tol)).any():
            warnings.warn(f"Initial state {state_name} is out of default range [{low}, {upp}]")