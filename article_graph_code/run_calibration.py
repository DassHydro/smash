...
def single_run_simulation(config):
    ...
    setup['start_time'] = '2015-08-01 00:00'
    setup['end_time'] = '2019-08-01 00:00'
    ...
    cal_code = pd.read_csv('/local/AIX/mettalbi/Projects/smash-maintenance-1.0.x/smash/factory/dataset/Arcmed/cal.csv')
    cal_code = cal_code["code"].to_list()
    ...
    if (config[0] == 'ann'):
        model.optimize(mapping='ann', cost_options={'gauge':cal_code, 'end_warmup': '2016-08-01'}, common_options = {"ncpu": 8, "verbose": True}, optimize_options={'termination_crit':{'epochs':setup['maxiter']}})  # end warmup & change start date
        # model.optimize(mapping='ann', optimize_options={'termination_crit':{'epochs':setup['maxiter']}})

        smash.io.save_model_ddt(model, 'results/models_calibrated/Arcmed_'+config[0]+'_'+config[1]+'_'+config[2]+'_sm_'+config[3]+'.hdf5')

    else :
        # model.optimize(mapping='distributed',  optimize_options={'termination_crit':{'maxiter':setup['maxiter']}})
        model.optimize(mapping='distributed', cost_options={'gauge':cal_code, 'end_warmup': '2016-08-01'}, common_options = {"ncpu": 8, "verbose": True},  optimize_options={'termination_crit':{'maxiter':setup['maxiter']}})

        smash.io.save_model(model, 'results/models_calibrated/Arcmed_'+config[0]+'_'+config[1]+'_'+config[2]+'_sm_'+config[3]+'.hdf5')
...
pool.close()
#single_run_simulation(configs[0])