...
:math:`t\in[1..T]` a time step.

First, the precipitation :math:`P` and the snow :math:`S` are summed to give to total precipitation :math:`P_T`

.. math::

    P_T(x, t) = P(x, t) + S(x, t) \;\;\; \forall (x, t) \in \Omega \times [1..T]
...
.. math::
    ...
    \begin{eqnarray}
        ...
        &S(x, t)& &=& &(1 - l_r(x, t)) \times P_T (x, t) \;\;\; \forall (x, t) \in \Omega \times [1..T]

    \end{eqnarray}

where the liquid ratio :math:`l_r` is derived from the classical parametric S-shaped curve :cite:p:`garavaglia2017impact`.

.. math::

    l_r(x, t) = 1 - \left( 1 + \exp\left( \frac{10}{4}T_e(x, t) - 1\right)\right)^{-1} \;\;\; \forall (x, t) \in \Omega \times [1..T]
