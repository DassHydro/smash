...
.. note::
    ...
    More information can be find in the User Guide section: :ref:`Model structure <user_guide.model_structure.gr_d>`.
...
.. note::
    ...
    More information can be find in the Math / Num Documentation section :ref:`math_num_documentation.hydrological_signature`.
...
only the main data. A set of default has been choosen in order to facilitate the use of the method but any kind of data can be extra saved.

.. note::
    
    More information can be find in the API reference section: :ref:`api_reference.io`.
...
A bug has been find in spatially uniform optimization with the ``sbs`` algorithm. If one non-optimized parameter is spatially distributed. 
...
A bug has been find in spatially distributed optimization. The gradients were not correctly computed with respect to the control vector normalization. 
